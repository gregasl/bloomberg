...
def write_req(prev_cusips : list):
    try:
        ...
        ASL.send_email('CONNECTION ERROR - Treasury - Request Builder', 'Hi, \n\nThere is a connection error with ASLFISSQL.', "brian.watson@aslcap.com")
    sql = """
    SELECT 
        BND.CUSIP_NUMBER, 
        BND.MATURITY_DATE, 
    ...
    df = df.reset_index(drop=True)
    df.to_csv(path + '/CUSIP.csv', index=False)

    try:
        req = open(path + '/treasury.req', 'r')
    except:
        ASL.send_email('FILE-READING ERROR - Treasury - Request Builder', 'Hi,\n\nThe program(write_req.py) cannot open file treasury.req in aslrisk01.', "tech@aslcap.com")
    file = req.read()
    req.close()
