...
class BloombergResponsePoller:
    def __init__(self, redis_host='localhost', redis_port=6379, redis_db=0,
        ...
        self.sql_conn_str = sql_server_conn_str or (
            ...
            "SERVER=localhost;"
            "DATABASE=BloombergAPI;"
            "Trusted_Connection=yes;"
        )
        
        # Bloomberg API configuration
        self.catalog = catalog or os.environ.get("BLOOMBERG_DL_ACCOUNT_NUMBER", "").removeprefix("DL ")
        self.client_id = client_id or os.environ.get('BLOOMBERG_DL_CLIENT_ID')
        self.client_secret = client_secret or os.environ.get('BLOOMBERG_DL_CLIENT_SECRET')
    ...
    def _initialize_database(self):
        """Initialize additional database tables for processing"""
        try:
            with pyodbc.connect(self.sql_conn_str) as conn:
                cursor = conn.cursor()
                
                # Create polling status table
                cursor.execute("""
                    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='polling_status' AND xtype='U')
                ...
                # Create processed responses log table
                cursor.execute("""
                    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='processed_responses' AND xtype='U')
                ...
                # Create data storage table for CSV responses
                cursor.execute("""
                    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='bloomberg_data' AND xtype='U')
                ...
                conn.commit()
                logger.info("Response poller database tables initialized successfully")
